...
const Cell: React.FC<Props> = ({children}) => {
    ...
    return <div onClick={() => setIsEditing(true)} className="hover:cursor-text border text-black flex items-center justify-center h-6">
        {isEditing ? <input onKeyDown={(k) => k.code === 'Enter' && setIsEditing(false) } placeholder={content} ref={userInput} onChange={(e) => setContent(e.target.value)} className="grow w-16 h-6 focus:outline-dashed p-0 m-0 text-center" type="text"/> : content}
    </div>
}
